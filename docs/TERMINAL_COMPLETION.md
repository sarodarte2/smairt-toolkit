...
requests, preserves the current menu selection, and returns one level with Left or Escape.
...
does not attempt to resize terminal text. Setup → Appearance stores themes and logos only in the
user-local SMAIRT configuration; it does not change a research project.
